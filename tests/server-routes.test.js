...
test("POST /api/chat/stream runs Codex profiles through the local Codex CLI and emits AI Chat SSE", async () => {
	...
	const spawnFn = (command, args) => {
		...
		process.nextTick(() => {
			...
			child.stdout.write('{"type":"item.started","item":{"id":"call_1","type":"command_execution","command":"/bin/zsh -lc \\"git status --short\\"","aggregated_output":"","exit_code":null,"status":"in_progress"}}\n');
			child.stdout.write('{"type":"item.completed","item":{"id":"call_1","type":"command_execution","command":"/bin/zsh -lc \\"git status --short\\"","aggregated_output":"M lib/server-runtime.js\\n","exit_code":0,"status":"completed"}}\n');
...
test("POST /api/chat/stream records Codex runs in the Watchdog requests intake", async () => {
	...
	const spawnFn = () => {
		...
		process.nextTick(() => {
			...
			child.stdout.write('{"type":"item.started","item":{"id":"call_1","type":"command_execution","command":"/bin/zsh -lc \\"git diff --stat\\"","aggregated_output":"","exit_code":null,"status":"in_progress"}}\n');
			child.stdout.write('{"type":"item.completed","item":{"id":"call_1","type":"command_execution","command":"/bin/zsh -lc \\"git diff --stat\\"","aggregated_output":"2 files changed\\n","exit_code":0,"status":"completed"}}\n');
...
test("POST /api/chat/stream routes Hermes profiles through the managed local proxy", async () => {
	const observed = [];
	const { runtime, destroy } = createIsolatedRuntime({
		envMerge: {
			HERMES_PROXY_URL: "http://127.0.0.1:8645/v1",
			HERMES_PROXY_TOKEN: "test-hermes-placeholder"
		},
		fetchFn: async (url, options) => {
			observed.push({ url, options });
			return mockSseResponse([{ choices: [{ delta: { content: "Hermes ok" }, finish_reason: "stop" }], usage: { prompt_tokens: 2, completion_tokens: 2, total_tokens: 4 } }]);
		}
	});
	try {
		const profile = runtime.helpers.readState().settings.profiles.find((entry) => entry.provider_id === "hermes");
		await withServer(runtime.app, async (baseUrl) => {
			const response = await fetch(`${baseUrl}/api/chat/stream`, {
				method: "POST",
				headers: withAuthHeaders({ "Content-Type": "application/json" }),
				body: JSON.stringify({ profile_id: profile.id, model: "stealth/ox-alpha", messages: [{ role: "user", content: "hi" }] })
			});
			const events = parseSseEvents(await response.text());
			assert.equal(events.find((entry) => entry.event === "done").data.output_text, "Hermes ok");
		});
		assert.equal(observed[0].url, "http://127.0.0.1:8645/v1/chat/completions");
		assert.equal(observed[0].options.headers.Authorization, "Bearer test-hermes-placeholder");
	} finally {
		destroy();
	}
});
