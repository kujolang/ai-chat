...
The default per-request tool budget is 24 tool rounds and 96 tool calls, configurable with `MAX_TOOL_ROUNDS` and `MAX_TOOL_CALLS_PER_REQUEST` and capped at 64 rounds / 256 calls to prevent runaway loops. If the cap is reached, the stream reports the active budget in its terminal error.
