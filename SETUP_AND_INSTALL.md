...
TOOL_CONTINUATION_TIMEOUT_MS=120000
...
Streaming Codex responses keep a native session for explicit continuation. Use **Review execution** on a saved response to inspect receipts, reconcile an uncertain outcome, recover running output, or resume interrupted work. Verify the native session history and external actions before reconciling a `native_attempt`; a process interruption can hide its last action. Resumption uses the saved session and configured sandbox. Legacy ephemeral sessions cannot be resumed. Codex manages its session files separately from AI Chat's encrypted journal; `EXECUTION_RETENTION_DAYS` controls only the latter. See [durable execution contracts](docs/API_CONTRACT.md#durable-execution-foundation).
...
- An authenticated xAI OAuth credential adds a separate managed `xAI Grok (X OAuth)` profile backed by the linked X/Grok subscription, not Nous Portal's free models. Run `hermes auth add xai-oauth --type oauth` once, then keep `hermes proxy start --provider xai --port 8646` running. Hermes acts only as the local OAuth bridge. AI Chat accepts only the exact loopback `XAI_OAUTH_PROXY_URL`; it never receives the xAI OAuth credential.
