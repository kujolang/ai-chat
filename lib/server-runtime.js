...
function createServerRuntime(options = {}) {
	...
	function inferFinishReason(finishReason, upstreamCompleted, usage, requestedMaxTokens, fullText) {
		...
		if (requestedMaxTokens > 0 && outputTokens >= requestedMaxTokens && responseLikelyTruncated(fullText)) {
			return "max_tokens";
		}
	...
	function chatRequestPayload(body, profile) {
		...
		const requestedMaxTokens = Number.isFinite(Number(body.max_tokens)) ? Number(body.max_tokens) : defaultMaxTokens;
