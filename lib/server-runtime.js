...
function buildProviderCatalog(codexCatalogModels = []) {
	return [
		...
		{
			id: "hermes_xai",
			label: "Hermes (xAI Grok OAuth)",
			models: hermesXaiCatalogModels
		},
		{
			id: "codex",
			label: "Codex",
...
function createServerRuntime(options = {}) {
	...
	const hermesXaiProxyUrl = String(env.HERMES_XAI_PROXY_URL || "http://127.0.0.1:8646/v1").trim().replace(/\/+$/, "");
	...
	function defaultProfiles() {
		...
		const profiles = [
			...
			{
				id: uid(),
				name: "Hermes / xAI Grok (OAuth)",
				provider_id: "hermes_xai",
				base_url: "",
				models_csv: hermesXaiCatalogModels.join(","),
				api_key: "",
				createdAt: stamp,
				updatedAt: stamp
			}
	...
	function applyModelCatalogMigration() {
		const catalogs = [
			...
			{ providerId: "hermes_xai", suggestions: hermesXaiCatalogModels },
	...
	function seedState() {
		...
		if (profileCount === 0) {
			...
			if (!db.prepare("SELECT id FROM profiles WHERE provider_id = 'hermes_xai' LIMIT 1").get()) {
				const nextSortOrder = Number((db.prepare("SELECT MAX(sort_order) AS value FROM profiles").get() || {}).value || 0) + 1;
				insertSeedProfile(defaultProfiles().find((profile) => profile.provider_id === "hermes_xai"), nextSortOrder);
			}
	...
	function validateStateShape(state) {
		...
		for (const profile of state.settings.profiles) {
			...
			if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter", "watchdog_ollama_tud", "hermes", "hermes_xai", "codex"].includes(providerId)) {
	...
	function validateStateChanges(payload) {
		...
		for (const change of payload.changes) {
			...
			if (entityName === "profile") {
				if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter", "watchdog_ollama_tud", "hermes", "hermes_xai", "codex"].includes(String(entity.provider_id || ""))) {
	...
	function isManagedCredentialProfile(profile) {
		return isManagedWatchdogProfile(profile) || isHermesProfile(profile) || isCodexProfile(profile);
	}
	...
	function isHermesProfile(profile) {
		return profile && (profile.provider_id === "hermes" || profile.provider_id === "hermes_xai");
	}
	...
	function effectiveApiKey(profile, watchdogRuntime = defaultWatchdogRuntime) {
		if (isHermesProfile(profile)) return hermesProxyToken;
	...
	function providerConfig(profile, watchdogRuntime = defaultWatchdogRuntime) {
		if (isHermesProfile(profile)) {
			return {
				base_url: profile.provider_id === "hermes_xai" ? hermesXaiProxyUrl : hermesProxyUrl,
	...
	function getProviderApiKeyEnvName(providerId) {
		...
		if (providerId === "watchdog" || providerId === "watchdog_openrouter" || providerId === "watchdog_ollama_tud" || providerId === "hermes" || providerId === "hermes_xai") {
	...
	function validateProviderBaseUrl(baseUrl, providerId, watchdogRuntime = defaultWatchdogRuntime) {
		...
		if (providerId === "hermes" || providerId === "hermes_xai") {
			const expectedProxyUrl = providerId === "hermes_xai" ? hermesXaiProxyUrl : hermesProxyUrl;
			const proxyEnvName = providerId === "hermes_xai" ? "HERMES_XAI_PROXY_URL" : "HERMES_PROXY_URL";
			const normalized = `${parsed.protocol}//${parsed.host}${parsed.pathname.replace(/\/+$/, "")}`;
			if (normalized !== expectedProxyUrl || parsed.username || parsed.password || parsed.search || parsed.hash) {
				throw new Error(`Hermes provider URL must match ${proxyEnvName}.`);
			}
			if (!(parsed.protocol === "http:" && ["127.0.0.1", "localhost", "[::1]"].includes(parsed.hostname.toLowerCase()))) {
				throw new Error("Hermes provider must use the configured loopback HTTP endpoint.");
			}
			return normalized;
		}
	...
	app.post("/api/chat", async (req, res) => {
		try {
			...
			const payload = {
				provider_id: isManagedWatchdogProfile(profile) || isHermesProfile(profile) ? "custom" : profile.provider_id,
				model: requestInfo.model,
