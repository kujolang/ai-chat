...
function createServerRuntime(options = {}) {
	...
	function readState() {
		...
		const profiles = profileRows.map((profile) => ({
			...
			credential_managed: profile.provider_id === "watchdog",
	...
	function validateStateShape(state) {
		...
		for (const profile of state.settings.profiles) {
			...
			if (!["openai", "deepseek", "openrouter", "custom", "watchdog"].includes(providerId)) {
	...
	function validateStateChanges(payload) {
		...
		for (const change of payload.changes) {
			...
			if (entityName === "profile") {
				if (!["openai", "deepseek", "openrouter", "custom", "watchdog"].includes(String(entity.provider_id || ""))) {
	...
	function profileById(profileId) {
		...
		return {
			...
			api_key: apiKey,
			credential_managed: row.provider_id === "watchdog"
		};
	}
	...
	function validateProviderBaseUrl(baseUrl, providerId) {
		...
		if (providerId === "watchdog") {
			...
			if (normalized !== watchdogProxyUrl || parsed.username || parsed.password || parsed.search || parsed.hash) {
				throw new Error("Watchdog provider URL must match WATCHDOG_PROXY_URL.");
			}
	...
	function watchdogObserveHeaders(body, profile) {
		if (profile.provider_id !== "watchdog") return {};
	...
	app.post("/api/chat", async (req, res) => {
		try {
			...
			const payload = {
				provider_id: profile.provider_id === "watchdog" ? "custom" : profile.provider_id,
		...
		try {
			...
			if (contentType.includes("text/event-stream")) {
				...
				const donePayload = {
					ok: true,
					trace_id: traceId,
					watchdog_trace: profile.provider_id === "watchdog",
					provider: profile.provider_id,
					model: modelName,
