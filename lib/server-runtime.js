...
function createServerRuntime(options = {}) {
	...
	const maxToolRounds = Math.min(parseInteger(env.MAX_TOOL_ROUNDS, 4), 8);
	const maxToolCallsPerRequest = Math.min(parseInteger(env.MAX_TOOL_CALLS_PER_REQUEST, 8), 32);
	...
	async function postWatchdogTelemetry(body, profile, result, latencyMs, transportMode) {
		if (!watchdogTelemetryUrl || profile.provider_id !== "watchdog" || transportMode !== "direct") {
			return;
		}
		...
		try {
			...
			const telemetryResponse = await fetchFn(watchdogTelemetryUrl, {
				method: "POST",
				headers,
				body: JSON.stringify({
					source_app: "ai-chat",
					request_id: String(body.correlation_id || body.pane_id || uid()),
					...
					prompt_summary: `message_chars=${String(Array.isArray(body.messages) && body.messages.length > 0 ? body.messages[body.messages.length - 1].content || "" : "").length}`,
					response_summary: `response_chars=${String(result.output_text || "").length}`
				}),
	...
	async function executeWebSearchTool(rawArguments, apiKey, signal) {
		if (!apiKey) {
			const error = new Error("Web Search requires an API-key-backed Ollama profile in Settings.");
			error.code = "tool_auth_error";
			throw error;
		}
		const args = normalizeToolCallArguments(rawArguments);
		let query = String(args.query || "").trim();
		if (!query || query.length > 500) {
			const error = new Error("web_search.query must contain between 1 and 500 characters.");
			error.code = "invalid_tool_arguments";
			throw error;
		}
		const recencyDays = Number(args.recency_days);
		if (Number.isInteger(recencyDays) && recencyDays > 0 && recencyDays <= 3650) {
			const cutoff = new Date(nowMs() - recencyDays * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
			query = `${query} after:${cutoff}`;
		}
		const requestedMax = Number(args.max_results);
		const maxResults = Number.isInteger(requestedMax)
			? Math.max(1, Math.min(requestedMax, webSearchMaxResults, 10))
			: webSearchMaxResults;
		...
		if (!response.ok) {
			const error = new Error(`Ollama Web Search returned HTTP ${response.status}.`);
		...
		try {
			...
			const invalid = new Error("Ollama Web Search returned invalid JSON.");
			invalid.code = "tool_execution_failed";
			throw invalid;
		}
		const results = (Array.isArray(payload.results) ? payload.results : []).slice(0, maxResults).map((entry) => ({
			title: String(entry && entry.title || "").slice(0, 500),
			url: String(entry && entry.url || "").slice(0, 2000),
			content: String(entry && entry.content || "").slice(0, 5000)
		}));
		return { query: String(args.query || "").trim(), results };
	}
		...
		try {
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					const unsupportedCalls = normalizedToolCalls.filter((call) => call.function.name !== "web_search");
					...
					if (toolRound >= maxToolRounds || executedToolCalls + normalizedToolCalls.length > maxToolCallsPerRequest) {
						sseEvent(res, "error", {
							...
							message: "The provider exceeded AI Chat's bounded web-search tool budget.",
							retryable: false
						});
						res.end();
						return;
					...
					try {
						for (const toolCall of normalizedToolCalls) {
							toolResults.push(await executeWebSearchTool(toolCall.function.arguments, searchApiKey, upstreamController.signal));
							armUpstreamTimeout();
						}
					} catch (error) {
						sseEvent(res, "error", {
							code: error && error.code ? error.code : "tool_execution_failed",
							message: error && error.message ? error.message : "Web Search failed.",
							retryable: false,
				...
				res.end();
				void postWatchdogTelemetry(body, profile, donePayload, nowMs() - streamStartedAt, transportMode);
				return;
			}
			} finally {
				if (upstreamTimeout) {
