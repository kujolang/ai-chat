...
function createServerRuntime(options = {}) {
	...
	app.use("/api", (req, res, next) => {
		...
		if (!isOriginAllowed(req, allowedOrigin)) {
		...
		if (!consumeRateLimit(rateBuckets, `${requestIp(req)}:${scope}`, rateLimitWindowMs, limit)) {
	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			if (contentType.includes("text/event-stream")) {
				...
				const processBufferedLines = (flushFinalLine = false) => {
					...
					for (const line of lines) {
						...
						if (eventObj.usage && typeof eventObj.usage === "object") {
							usage = {
								input_tokens: Number(eventObj.usage.prompt_tokens || 0),
								output_tokens: Number(eventObj.usage.completion_tokens || 0),
								total_tokens: Number(eventObj.usage.total_tokens || 0)
							};
						}
					}
				};
				...
				fullText = flattenText(message && message.content ? message.content : "");
				thinkingText = flattenText(message && (message.reasoning_content || message.reasoning || message.thinking)
...
function isOriginAllowed(req, explicitAllowedOrigin) {
...
function consumeRateLimit(bucketMap, key, windowMs, limit) {
	const now = Date.now();
