...
function createServerRuntime(options = {}) {
	...
	if (watchdogProxyTokenFile) {
		try {
			...
			console.warn("[watchdog] Proxy token file is configured but unavailable.");
		}
	}
	if (watchdogApiTokenFile) {
		try {
			...
			console.warn("[watchdog] API token file is configured but unavailable.");
		}
	}
	...
	async function postWatchdogTelemetry(body, profile, result, latencyMs, transportMode) {
		...
		try {
			...
			await fetchFn(watchdogTelemetryUrl, {
				method: "POST",
				headers,
				body: JSON.stringify({
					source_app: "ai-chat",
			...
			// Telemetry is best effort and must never delay or fail the chat stream.
		} finally {
			clearTimeout(timeout);
		}
	}
	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (requestedToolCalls.length > 0 || finishReason === "tool_calls") {
