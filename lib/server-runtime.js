...
function createServerRuntime(options = {}) {
	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			const persistDetachedStreamCheckpoint = (entry = {}) => {
			...
			const requestToolAllowlist = effectiveToolAllowlist(requestInfo.tools);
			const expectedProviderToolNames = [...new Set(requestInfo.tools.flatMap((tool) => {
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					const authorizationResults = normalizedToolCalls.map((call) => authorizeToolCall(call, requestToolAllowlist));
					...
					if (unauthorizedCalls.length > 0) {
						...
						sseEvent(res, "error", toolExecutionUnavailablePayload(unauthorizedCalls));
						res.end();
						return;
					}
				...
				if (isStandaloneTextToolCall(aggregateFullText)) {
					...
					sseEvent(res, "error", malformedToolCall);
					res.end();
					return;
				}
			...
			try {
				const message = toolContinuationTimedOut
					? "The provider did not continue after a tool result within the configured timeout. Retry the request."
					: upstreamTimedOut
						? "The upstream stream timed out before completion. Increase STREAM_REQUEST_TIMEOUT_MS or retry."
						: transportErrorPayload.message;
				persistDetachedStreamCheckpoint({
					output_text: "",
					thinking_text: "",
					usage: { error: { message, retryable: true } },
				...
				// Persistence is best-effort for detached streams; the stream error still wins.
			}
			if (clientDisconnected) {
				return;
			}
	...
	return {
		structured_excerpt_v1(messages, options = {}) {
			...
			const lines = [
				...
				"The following summary replaces older turns for this request only. Prefer the newer verbatim turns when they conflict."
			];
			...
			for (const role of ["user", "assistant", "tool", "system", "unknown"]) {
				...
				for (const message of bucket.slice(-8)) {
...
function summarizeCodexToolArguments(toolName, rawArguments) {
	...
	if (toolName === "command_execution") {
		summary.command = String(args.command || "").slice(0, 400);
