...
function createServerRuntime(options = {}) {
	...
	const streamRequestTimeoutMs = parseTimeoutMs(env.STREAM_REQUEST_TIMEOUT_MS, 0);
	const toolContinuationTimeoutMs = parseTimeoutMs(env.TOOL_CONTINUATION_TIMEOUT_MS, streamRequestTimeoutMs);
	...
	app.use((req, res, next) => {
		...
		res.setHeader(
			...
			"default-src 'self'; script-src 'self'; connect-src 'self'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; font-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
		);
