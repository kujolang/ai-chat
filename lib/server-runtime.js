				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					for (const [toolCallIndex, toolCall] of normalizedToolCalls.entries()) {
							...
							try {
								...
								const repairSummary = summarizeToolInputRepairs(toolResult && toolResult.input_repairs);
								if (repairSummary.count > 0) {
									addTraceEvent("tool_input_repaired", toolSpan, { tool_name: toolName, ...repairSummary });
									audit("tool_input_repaired", req, { tool_name: toolName, ...repairSummary });
								}
								...
								sseEvent(res, "tool", {
									phase: "completed",
									tool_name: toolName,
									input_repaired: repairSummary.count > 0,
								...
								const repairSummary = summarizeToolInputRepairs(error && error.input_repairs);
								if (repairSummary.count > 0) {
									addTraceEvent("tool_input_repaired", toolSpan, { tool_name: toolName, ...repairSummary });
									audit("tool_input_repaired", req, { tool_name: toolName, ...repairSummary });
								}
								...
								toolResults.push({
									...
									...(repairSummary.count > 0 ? { input_repairs: repairSummary } : {})
								});
							}
							armUpstreamTimeout();
						}
...
function summarizeToolResult(toolName, result, mode = "off") {
	...
	const summary = {
		...
		input_repairs: summarizeToolInputRepairs(result && result.input_repairs)
	};
...
function summarizeToolInputRepairs(value) {
	const repairs = Array.isArray(value) ? value.slice(0, 24) : [];
	return {
		count: repairs.length,
		kinds: [...new Set(repairs.map((entry) => String(entry && entry.kind || "").slice(0, 80)).filter(Boolean))].slice(0, 12),
		paths: [...new Set(repairs.map((entry) => String(entry && entry.path || "").slice(0, 120)).filter(Boolean))].slice(0, 12)
	};
}
