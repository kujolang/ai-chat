...
function createServerRuntime(options = {}) {
	...
	function validateStateShape(state) {
		...
		for (const profile of state.settings.profiles) {
			...
			if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter", "watchdog_ollama_tud", "hermes", "codex"].includes(providerId)) {
	...
	function validateStateChanges(payload) {
		...
		for (const change of payload.changes) {
			...
			if (entityName === "profile") {
				if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter", "watchdog_ollama_tud", "hermes", "codex"].includes(String(entity.provider_id || ""))) {
	...
	function effectiveApiKey(profile, watchdogRuntime = defaultWatchdogRuntime) {
		if (profile.provider_id === "hermes") return hermesProxyToken;
	...
	function getProviderApiKeyEnvName(providerId) {
		...
		if (providerId === "watchdog" || providerId === "watchdog_openrouter" || providerId === "watchdog_ollama_tud" || providerId === "hermes") {
	...
	function validateProviderBaseUrl(baseUrl, providerId, watchdogRuntime = defaultWatchdogRuntime) {
		...
		if (providerId === "hermes") {
			const normalized = `${parsed.protocol}//${parsed.host}${parsed.pathname.replace(/\/+$/, "")}`;
			if (normalized !== hermesProxyUrl || parsed.username || parsed.password || parsed.search || parsed.hash) {
				throw new Error("Hermes provider URL must match HERMES_PROXY_URL.");
			}
			if (!(parsed.protocol === "http:" && ["127.0.0.1", "localhost", "[::1]"].includes(parsed.hostname.toLowerCase()))) {
