		...
		try {
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					const toolResults = [];
					clearUpstreamTimeout();
					executionJournal.checkpoint(streamRequestId, { messages: conversationMessages, pending_calls: normalizedToolCalls, round_text: fullText, round_thinking: thinkingText, output_text: aggregateFullText, thinking_text: aggregateThinkingText, usage: aggregateUsage, executed_calls: executedToolCalls + executableToolCalls });
					...
					executionJournal.checkpoint(streamRequestId, { messages: conversationMessages, output_text: aggregateFullText, thinking_text: aggregateThinkingText, usage: aggregateUsage, executed_calls: executedToolCalls });
					armUpstreamTimeout();
