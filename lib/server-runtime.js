...
function createServerRuntime(options = {}) {
	...
	function chatRequestPayload(body, profile) {
		...
		const messages = boundMessagesForRequest(normalizedMessages);
	...
	function boundMessagesForRequest(messages) {
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					for (const toolCall of normalizedToolCalls) {
							...
							sseEvent(res, "tool", { phase: "started", tool_name: toolName, activity: toolActivityLabel(toolName, toolCall.function.arguments) });
							try {
								...
								sseEvent(res, "tool", { phase: "completed", tool_name: toolName });
								traceToolCalls.push({
									tool_name: toolName,
									arguments: argumentSummary,
								...
								sseEvent(res, "tool", { phase: "failed", tool_name: toolName, error_code: String(error && error.code || "tool_execution_failed") });
