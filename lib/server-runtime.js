...
// Static suggestions from the 2026-07-17 catalog snapshot. They are selectable
...
const ollamaCloudCatalogModels = [
	...
	"qwen3.5:35b", "qwen3.5:122b", "qwen3.5:397b", "glm-5.1", "minimax-m2.7",
	"nemotron-3-super:120b", "minimax-m2.5", "glm-5.2", "minimax-m3", "kimi-k2.7-code",
	"kimi-k2.6", "deepseek-v4-flash", "deepseek-v4-pro", "kimi-k2.5", "nemotron-3-ultra",
	"gpt-oss:20b", "gpt-oss:120b", "gemini-3-flash-preview", "nemotron-3-nano:4b",
	"nemotron-3-nano:30b", "mistral-large-3:675b"
];
...
function createServerRuntime(options = {}) {
	...
	const watchdogOllamaTudProxyUrl = String(env.WATCHDOG_OLLAMA_TUD_PROXY_URL || "http://127.0.0.1:7702/proxy/v1").trim().replace(/\/+$/, "");
	const watchdogOllamaTudProxyTokenFile = String(env.WATCHDOG_OLLAMA_TUD_PROXY_TOKEN_FILE || "").trim();
	...
	let watchdogOllamaTudProxyToken = "";
	...
	if (watchdogOllamaTudProxyTokenFile) {
		try {
			watchdogOllamaTudProxyToken = fs.readFileSync(watchdogOllamaTudProxyTokenFile, "utf8").trim();
		} catch (error) {
			warnFn("[watchdog] Ollama TUD proxy token file is configured but unavailable.");
		}
	}
	...
	const maxToolRounds = Math.min(parseInteger(env.MAX_TOOL_ROUNDS, 8), 8);
	const maxToolCallsPerRequest = Math.min(parseInteger(env.MAX_TOOL_CALLS_PER_REQUEST, 24), 32);
	...
	const browserMaxSessions = parseInteger(env.BROWSER_MAX_SESSIONS, 4);
	const browserMaxSessionsPerChat = parseInteger(env.BROWSER_MAX_SESSIONS_PER_CHAT, 2);
	const browserMaxActionsPerRequest = parseInteger(env.BROWSER_MAX_ACTIONS_PER_REQUEST, 12);
	const browserMaxActionsPerSession = parseInteger(env.BROWSER_MAX_ACTIONS_PER_SESSION, 30);
	...
	app.use((req, res, next) => {
		...
		res.setHeader(
			...
			"default-src 'self'; script-src 'self' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; connect-src 'self'; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; font-src 'self' https://fonts.gstatic.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
		);
	...
	function applyModelCatalogMigration() {
		...
		for (const catalog of catalogs) {
			const profiles = db.prepare("SELECT id, models_csv FROM profiles WHERE provider_id = ?").all(catalog.providerId);
			for (const profile of profiles) {
				const modelsCsv = mergeModelSuggestions(profile.models_csv, catalog.suggestions);
	...
	function seedState() {
		...
		`);
		const insertSeedProfile = (profile) => {
		...
		} else if (!db.prepare("SELECT id FROM profiles WHERE provider_id = 'watchdog_ollama_tud' LIMIT 1").get()) {
			insertSeedProfile(defaultProfiles().find((profile) => profile.provider_id === "watchdog_ollama_tud"));
		}
	...
	function validateStateShape(state) {
		...
		for (const profile of state.settings.profiles) {
			...
			if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter", "watchdog_ollama_tud"].includes(providerId)) {
	...
	function validateStateChanges(payload) {
		...
		for (const change of payload.changes) {
			...
			if (change.type === "app_settings_upsert") {
				...
				if (Object.prototype.hasOwnProperty.call(settings, "paneProfiles")) {
					validatePaneProfiles(settings.paneProfiles);
				}
			...
			if (entityName === "profile") {
				if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter", "watchdog_ollama_tud"].includes(String(entity.provider_id || ""))) {
	...
	function validatePaneProfiles(paneProfiles) {
		...
		for (const paneProfile of paneProfiles) {
			...
			if (!Array.isArray(paneProfile.panes) || paneProfile.panes.length === 0 || paneProfile.panes.length > 12) {
	...
	const applyStateChanges = db.transaction((payload) => {
		...
		for (const change of payload.changes) {
			...
			if (change.type === "chat_upsert") {
				...
				db.prepare(`
					INSERT INTO chats (id, title, project_path, pinned, archived, created_at, updated_at, sort_order)
					VALUES (?, ?, ?, ?, ?, ?, ?, ?)
					ON CONFLICT(id) DO UPDATE SET
			...
			if (change.type === "app_settings_upsert") {
				const settings = change.settings;
				const existingSettings = db.prepare("SELECT pane_profiles_json, agent_instructions, agent_instruction_profiles_json FROM app_settings WHERE id = 1").get();
				const paneProfilesJson = Array.isArray(settings.paneProfiles)
					? JSON.stringify(settings.paneProfiles)
					: String((existingSettings && existingSettings.pane_profiles_json) || "[]");
	...
	function profileById(profileId) {
		...
		return {
			...
			api_key: apiKey,
			credential_managed: isManagedWatchdogProfile(row)
		};
	}
	...
	function effectiveApiKey(profile) {
		...
		if (profile.provider_id === "watchdog_ollama_tud") return watchdogOllamaTudProxyToken;
	...
	function providerConfig(profile) {
		...
		if (profile.provider_id === "watchdog_ollama_tud") {
			return {
				base_url: watchdogOllamaTudProxyUrl,
				chat_path: "/chat/completions",
				transcribe_path: null,
				watchdog_managed: true
			};
	...
	function validateProviderBaseUrl(baseUrl, providerId) {
		...
		if (providerId === "watchdog" || providerId === "watchdog_openrouter" || providerId === "watchdog_ollama_tud") {
			const expectedProxyUrl = providerId === "watchdog_ollama_tud" ? watchdogOllamaTudProxyUrl : watchdogProxyUrl;
	...
	async function postWatchdogTelemetry(body, profile, result, latencyMs, transportMode, traceBundle = null) {
		if (!watchdogTelemetryUrl || profile.provider_id !== "watchdog") {
			return;
		}
		...
		try {
			...
			const targetUrl = transportMode === "direct"
		...
		try {
			...
			if (requestInfo.tools.some((tool) => ["browser_open", "browser_snapshot", "browser_act", "browser_close", "browser_use"].includes(tool.function.name))) {
				conversationMessages.unshift({
					role: "system",
					content: "Browser tool rules: use browser_use with action screenshot and the target URL for a screenshot request. This is a single safe read-only call. For multi-step browser_use actions, only use snapshot, extract, screenshot, click, type, scroll, back, or close with a session_id returned by an earlier successful open. Never use web_search as a substitute for browser access."
				});
			}
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					if (toolRound >= maxToolRounds || executedToolCalls + normalizedToolCalls.length > maxToolCallsPerRequest) {
					...
					try {
						for (const toolCall of normalizedToolCalls) {
							...
							try {
								...
								traceToolCalls.push({ tool_name: toolName, arguments: argumentSummary, result: { error_code: String(error && error.code || "tool_execution_failed") }, status: "error", latency_ms: Math.max(0, nowMs() - toolStartedAt) });
								throw error;
							}
							armUpstreamTimeout();
						}
					} catch (error) {
						finalizeAndPostTrace("error", { ok: false, model: modelName, usage: aggregateUsage, output_text: aggregateFullText, thinking_text: aggregateThinkingText, finish_reason: "tool_calls", error_code: error && error.code ? error.code : "tool_execution_failed", error_message: error && error.message ? error.message : "Tool execution failed." }, { failure_stage: "tool_execution" });
						sseEvent(res, "error", {
					...
					executedToolCalls += normalizedToolCalls.length;
				...
				if (typeof finalizeStreamTrace === "function") {
					finalizeStreamTrace("error", { ok: false, model: req.body && req.body.model || "", finish_reason: "error", error_code: toolContinuationTimedOut ? "tool_continuation_timeout" : (upstreamTimedOut ? "stream_timeout" : "stream_failed"), error_message: String(error && error.message || "") }, { failure_stage: toolContinuationTimedOut ? "tool_continuation_timeout" : (upstreamTimedOut ? "stream_timeout" : "stream_transport") });
				}
...
function summarizeToolArguments(toolName, rawArguments, mode = "off") {
	...
	if (toolName.startsWith("browser_")) {
		...
		try { summary.destination_host = action.url ? new URL(String(action.url)).hostname : ""; } catch (error) { summary.destination_host = ""; }
	}
