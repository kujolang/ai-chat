	...
	async function executeAutomationChat(context) {
		...
		const processEvent = () => {
			...
			if (eventName === "tool") {
				...
				const failedReason = payload.phase === "failed" && payload.error_reason ? ` — ${payload.error_reason}` : "";
				toolActivity.push(payload.phase === "started" ? (activity || `Using ${toolName}…`) : `${activity || `Used ${toolName}`} · ${payload.phase}${failedReason}`);
			}
	...
	function runCodexExec({ messages, model, onEvent }) {
		return new Promise((resolve) => {
			...
			try {
				...
				finalize({ ok: false, error, stdout, stderr, status: null, finalText, usage, threadId });
				return;
			}
			...
			child.on("error", (error) => finalize({ ok: false, error, stdout, stderr, status: null, finalText, usage, threadId }));
			child.on("close", (status) => {
				...
				finalize({ ok: true, error: null, stdout, stderr, status: Number.isFinite(status) ? status : null, finalText, usage, threadId });
			});
		});
	}
		...
		try {
			...
			if (isCodexProfile(profile)) {
				...
				if (!codexResult.ok) {
					...
					}, Math.max(0, nowMs() - streamStartedAt), "local", { trace_id: traceId });
					sseEvent(res, "error", {
				...
				void postWatchdogTelemetry(body, profile, donePayload, Math.max(0, nowMs() - streamStartedAt), "local", { trace_id: traceId });
				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					for (const [toolCallIndex, toolCall] of normalizedToolCalls.entries()) {
							...
							try {
								...
								sseEvent(res, "tool", { phase: "failed", tool_name: toolName, error_code: toolErrorCode, error_reason: toolErrorReason(toolErrorCode) });
