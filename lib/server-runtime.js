	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			for (let toolRound = 0; toolRound <= maxToolRounds; toolRound += 1) {
				...
				const contextBudget = measureModelContext(conversationMessages, loadedSchemas);
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (normalizedToolCalls.length > 0 || finishReason === "tool_calls") {
					...
					for (const [toolCallIndex, toolCall] of normalizedToolCalls.entries()) {
						...
						if (toolCallIndex >= executableToolCalls) {
							...
							try {
								const toolResult = toolName === "tool_discover"
				...
				if (["stream_cancelled", "stream_capacity", "stream_already_running", "invalid_request", "provider_record_too_large", "provider_response_too_large"].includes(error && error.code)) {
