...
function createServerRuntime(options = {}) {
	...
	const defaultMaxTokens = parseInteger(env.DEFAULT_MAX_TOKENS, 3200);
		...
		if (!settings) {
			db.prepare(`
				INSERT INTO app_settings (
					...
					active_chat_id, project_folders_json, show_archived, search_query, updated_at
	...
	const writeState = db.transaction((nextState) => {
		...
		db.prepare(`
			INSERT INTO app_settings (
				id, temperature, max_tokens, broadcast_to_all_panes,
				active_chat_id, project_folders_json, show_archived, search_query, state_version, updated_at
			) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	function profileById(profileId) {
		...
		const apiKey = decryptValue(row.api_key_cipher, row.api_key_iv, row.api_key_tag);
		return {
			id: row.id,
			name: row.name,
	...
	app.post("/api/chat", async (req, res) => {
		try {
			...
			res.status(500).json({ ok: false, error: { code: "chat_failed", message: error.message, retryable: false } });
		}
	});
	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			if (contentType.includes("text/event-stream")) {
				...
				const processBufferedLines = (flushFinalLine = false) => {
					...
					for (const line of lines) {
						...
						if (eventObj.error) {
							sseEvent(res, "error", {
								code: "provider_error",
								message: eventObj.error.message || "Provider stream error"
							});
							continue;
						}
						...
						if (eventObj.usage && typeof eventObj.usage === "object") {
							const inputTokens = Number(eventObj.usage.prompt_tokens || eventObj.usage.input_tokens || eventObj.prompt_eval_count || 0);
							const outputTokens = Number(eventObj.usage.completion_tokens || eventObj.usage.output_tokens || eventObj.eval_count || 0);
							usage = {
								input_tokens: inputTokens,
								output_tokens: outputTokens,
								total_tokens: Number(eventObj.usage.total_tokens || inputTokens + outputTokens)
							};
						}
					}
				};
				...
				for (const data of records) {
					...
					if (data.usage && typeof data.usage === "object") {
						const inputTokens = Number(data.usage.prompt_tokens || data.usage.input_tokens || data.prompt_eval_count || 0);
						const outputTokens = Number(data.usage.completion_tokens || data.usage.output_tokens || data.eval_count || 0);
						usage = {
							input_tokens: inputTokens,
							output_tokens: outputTokens,
							total_tokens: Number(data.usage.total_tokens || inputTokens + outputTokens)
						};
					}
				}
			...
			if (error && (error.name === "AbortError" || String(error.message || "").toLowerCase().includes("abort"))) {
				...
				sseEvent(res, "error", { code: "stream_failed", message: error.message });
			}
			res.end();
		}
	});
