...
function createServerRuntime(options = {}) {
	...
	for (const id of recoverableIds) {
		...
		persistStreamMessage({ chatId: request.chat_id, paneId: request.pane_id, messageId: request.assistant_message_id, content: text, thinking, provider: request.provider_id || "", model: request.model, status: run.status === "completed" ? "idle" : text || thinking ? "partial" : "error", usage: run.status === "completed" ? run.result?.usage : { error: { code: "execution_interrupted", message: "Execution interrupted by restart. Inspect saved receipts and resume explicitly.", retryable: false }, execution_id: id } });
	}
