...
function createServerRuntime(options = {}) {
	...
	function runCodexExec({ messages, model, onEvent, signal, contextBudget }) {
		signal?.throwIfAborted();
		return new Promise((resolve) => {
	...
	app.get("/api/executions/:id", (req, res) => {
		...
		res.json({ ok: true, execution: run, receipts: executionJournal.receipts(run.id) });
	});
	...
	app.get("/api/executions/:id/events", (req, res) => {
		...
		res.json({ ok: true, status: run.status, events, next_cursor: events.at(-1)?.sequence || after });
	});
	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			const requestInfo = chatRequestPayload(body, profile);
			const executionRequest = { provider_id: profile.provider_id, chat_id: String(body.chat_id || ""), pane_id: String(body.pane_id || ""), assistant_message_id: String(body.assistant_message_id || ""), profile_id: profileId, model: requestInfo.model, messages: requestInfo.messages, tools: requestInfo.tools, temperature: requestInfo.temperature, max_tokens: requestInfo.max_tokens, tool_discovery: body.tool_discovery === true };
			const turnId = String(body.assistant_message_id || streamRequestId);
			if (profile.provider_id === "codex" && body.resume === true && executionJournal.get(streamRequestId)?.status !== "completed") {
				throw Object.assign(new Error("Interrupted Codex runs require native session continuation; restarting their input could repeat actions."), { code: "execution_resume_unavailable", retryable: false });
			}
			...
			let aggregateUsage = resumedCheckpoint?.usage || null;
			providerRoundCount = resumedCheckpoint?.provider_rounds || 0;
			providerEvidence = { ...providerEvidence, provider_rounds: providerRoundCount, usage_reported_rounds: resumedCheckpoint?.usage_reported_rounds || 0, usage: aggregateUsage };
