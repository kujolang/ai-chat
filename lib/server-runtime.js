...
function createServerRuntime(options = {}) {
	...
	const watchdogOpenRouterProxyUrl = String(env.WATCHDOG_OPENROUTER_PROXY_URL || "http://127.0.0.1:7701/proxy/v1").trim().replace(/\/+$/, "");
	const watchdogOpenRouterProxyTokenFile = String(env.WATCHDOG_OPENROUTER_PROXY_TOKEN_FILE || "").trim();
	...
	let watchdogOpenRouterProxyToken = "";
	...
	if (watchdogOpenRouterProxyTokenFile) {
		try {
			watchdogOpenRouterProxyToken = fs.readFileSync(watchdogOpenRouterProxyTokenFile, "utf8").trim();
		} catch (error) {
			warnFn("[watchdog-openrouter] Proxy token file is configured but unavailable.");
		}
	}
	...
	function validateStateShape(state) {
		...
		for (const profile of state.settings.profiles) {
			...
			if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter"].includes(providerId)) {
	...
	function validateStateChanges(payload) {
		...
		for (const change of payload.changes) {
			...
			if (entityName === "profile") {
				if (!["openai", "deepseek", "openrouter", "custom", "watchdog", "watchdog_openrouter"].includes(String(entity.provider_id || ""))) {
	...
	const writeState = db.transaction((nextState) => {
		...
		db.prepare(`
			INSERT INTO app_settings (
				id, temperature, max_tokens, broadcast_to_all_panes,
				active_chat_id, project_folders_json, pane_profiles_json, tools_json, show_archived, search_query, state_version, updated_at
			) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	const applyStateChanges = db.transaction((payload) => {
		...
		for (const change of payload.changes) {
			...
			if (change.type === "app_settings_upsert") {
				const settings = change.settings;
				const existingSettings = db.prepare("SELECT pane_profiles_json FROM app_settings WHERE id = 1").get();
	...
	function isManagedWatchdogProfile(profile) {
		return profile && (profile.provider_id === "watchdog" || profile.provider_id === "watchdog_openrouter");
	}

	function effectiveApiKey(profile) {
		if (profile.provider_id === "watchdog") return watchdogProxyToken;
		if (profile.provider_id === "watchdog_openrouter") return watchdogOpenRouterProxyToken;
	...
	function providerConfig(profile) {
		...
		if (profile.provider_id === "watchdog_openrouter") {
			return {
				base_url: watchdogOpenRouterProxyUrl,
				chat_path: "/chat/completions",
				transcribe_path: null,
				watchdog_managed: true
			};
	...
	function getProviderApiKeyEnvName(providerId) {
		if (providerId === "watchdog" || providerId === "watchdog_openrouter") {
	...
	function validateProviderBaseUrl(baseUrl, providerId) {
		...
		if (providerId === "watchdog" || providerId === "watchdog_openrouter") {
			const expectedProxyUrl = providerId === "watchdog" ? watchdogProxyUrl : watchdogOpenRouterProxyUrl;
	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			const profile = profileById(profileId);
			const requestInfo = chatRequestPayload(body, profile);
			const browserScopeId = String(body.chat_id || body.session_id || body.request_id || req.request_id || "").slice(0, 300);
			...
			if (contentType.includes("text/event-stream")) {
				...
				if (typeof finalizeStreamTrace === "function") {
					finalizeStreamTrace("error", { ok: false, model: req.body && req.body.model || "", finish_reason: "error", error_code: upstreamTimedOut ? "stream_timeout" : "stream_failed", error_message: String(error && error.message || "") }, { failure_stage: upstreamTimedOut ? "stream_timeout" : "stream_transport" });
				}
