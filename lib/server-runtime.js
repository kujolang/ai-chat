...
const { contextPolicy, budgetContext } = require("./context-budget");
...
function createServerRuntime(options = {}) {
	...
	function runCodexExec({ messages, model, onEvent, signal }) {
		signal?.throwIfAborted();
		return new Promise((resolve) => {
	...
	app.post("/api/chat", async (req, res) => {
		...
		try {
			...
			const code = lifecycle?.signal.aborted ? lifecycle.signal.reason.code : (["auth_error", "invalid_request", "server_shutting_down", "stream_cancelled", "stream_capacity", "benchmark_saturated", "benchmark_queue_timeout"].includes(error.code) ? error.code : "chat_failed");
			res.status(code === "stream_cancelled" ? 499 : code === "server_shutting_down" ? 503 : code === "auth_error" || code === "invalid_request" ? 400 : 500).json({ ok: false, error: { code, message: error.message, retryable: false } });
		} finally {
			releaseSlot?.();
			lifecycle?.close();
		}
	...
	app.post("/api/chat/stream", async (req, res) => {
		...
		try {
			...
			for (let toolRound = 0; toolRound <= maxToolRounds; toolRound += 1) {
				...
				const modelContextPolicy = contextPolicy(profile.provider_id, requestInfo.model, modelContextLimits);
				const enforcement = budgetContext(conversationMessages, loadedSchemas, { ...modelContextPolicy, output_tokens: requestInfo.max_tokens });
				const contextBudget = { ...measureModelContext(conversationMessages, loadedSchemas), ...enforcement, context_window_tokens: modelContextPolicy.window_tokens, context_policy_source: modelContextPolicy.source };
