	...
	async function executeAutomationChat(context) {
		...
		const processEvent = () => {
			...
			if (eventName === "tool") {
				...
				const toolName = String(payload.tool_name || "tool").replaceAll("_", " ");
				const activity = String(payload.activity || "").trim();
				toolActivity.push(payload.phase === "started" ? (activity || `Using ${toolName}…`) : `${activity || `Used ${toolName}`} · ${payload.phase}`);
			}
		...
		return {
			...
			tool_activity: Array.from(new Set(toolActivity)),
