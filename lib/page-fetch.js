...
function createPageFetchRuntime(options = {}) {
	...
	async function execute(args, context = {}) {
		...
		try {
			...
			const response = await Promise.race([client.safeRequest(args.url, { method: "GET", headers: { accept: "text/markdown, text/html;q=0.9, text/plain;q=0.8", "user-agent": "AI-Chat-Page-Reader/1" } }, 0, signal), cancellation]);
			signal.throwIfAborted();
			...
			if (!/^text\/(markdown|html|plain)(?:;|$)/.test(type)) throw failure("web_fetch_content_type", "Page extraction supports Markdown, HTML and plain text. Use a suitable document tool for other formats.");
