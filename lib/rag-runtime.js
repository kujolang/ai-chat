...
function createRagRuntime({ env = process.env, fetchFn = fetch } = {}) {
 ...
 return {
  ...
  async execute(args, context = {}) {
   ...
   const signal = AbortSignal.any([AbortSignal.timeout(10000), ...(context.signal ? [context.signal] : [])]);
   let response;
   ...
   const citations = envelope.data.citations.map((citation) => {
    ...
    return { chunk_id: citation.chunk_id, path: citation.path, line_start: citation.line_start, line_end: citation.line_end, text: citation.text };
   });
