...
function createAutomationService(options) {
	...
	function initSchema() {
		...
		if (!columns.some((column) => String(column.name || "") === "tools_json")) {
			db.prepare("ALTER TABLE automations ADD COLUMN tools_json TEXT NOT NULL DEFAULT '[]'").run();
		}
	...
	function validate(input) {
		...
		const tools = normalizeToolNames(input && input.tools);
		...
		return { title, prompt, profileId, model, projectPath, tools, repeat, time, weekday, timezone, enabled: input.enabled !== false };
	}
	...
	function create(input) {
		...
		db.prepare(`
			INSERT INTO automations (
				id, title, prompt, profile_id, model, project_path, tools_json, repeat_kind,
				time_local, weekday, timezone, enabled, next_run_at, created_at, updated_at
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`).run(id, value.title, value.prompt, value.profileId, value.model, value.projectPath, JSON.stringify(value.tools),
			value.repeat, value.time, value.weekday, value.timezone, value.enabled ? 1 : 0, nextRunAt, stamp, stamp);
		return get(id);
	}
	...
	function update(id, input) {
		...
		const nextRunAt = value.enabled ? nextScheduledAt(value, stamp) : null;
		db.prepare(`
			UPDATE automations SET title = ?, prompt = ?, profile_id = ?, model = ?, project_path = ?, tools_json = ?,
				repeat_kind = ?, time_local = ?, weekday = ?, timezone = ?, enabled = ?, next_run_at = ?, updated_at = ?
			WHERE id = ?
		`).run(value.title, value.prompt, value.profileId, value.model, value.projectPath, JSON.stringify(value.tools), value.repeat,
...
function normalizeToolNames(value) {
	...
	return [...new Set(source.map((name) => String(name || "").trim())
		.filter((name) => /^[A-Za-z0-9_-]{1,64}$/.test(name)))].slice(0, 32);
}

function parseToolNames(value) {
	try {
		return normalizeToolNames(JSON.parse(String(value || "[]")));
	} catch (error) {
		return [];
	}
}
...
function nextScheduledAt(schedule, fromMs) {
	...
	const fromParts = partsFor(formatter, Number(fromMs));
	const targetReachedOnStartDate = (fromParts.hour * 60) + fromParts.minute >= (targetHour * 60) + targetMinute;
	for (let offset = 0; offset <= maxMinutes; offset += 1) {
		...
		if (targetReachedOnStartDate && sameCalendarDate(parts, fromParts)) continue;
