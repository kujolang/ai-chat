...
function createToolRuntime(options = {}) {
	...
	register("web_search", async (rawArguments, context = {}) => {
		...
		const searchPromise = (async () => {
			try {
				const payload = await executeSearchWithControls({
					fetchFn,
					nowMs,
					backend,
		...
		return clonePayload(await waitForSearchCaller(searchPromise, context.signal));
	});
...
async function waitForSearchCaller(searchPromise, signal) {
	...
	try {
		return await Promise.race([searchPromise, cancellation]);
	} finally {
