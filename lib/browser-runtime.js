...
function createBrowserRuntime(options = {}) {
		...
		if ((action === "extract" || action === "snapshot" || action === "screenshot") && !args.session_id && args.url) {
			...
			try {
				if (action === "screenshot") {
					return await act({ session_id: opened.session_id, action: { type: "screenshot" } }, scopeId, requestState);
				}
				return await snapshot({ session_id: opened.session_id }, scopeId, requestState);
			} finally {
	...
	async function handleRoute(route, session) {
		...
		if (!(method === "GET" || method === "HEAD")) {
			// Pages often emit telemetry or other background writes during initial
			// rendering. Keep the browser read-only by aborting them, but do not
			// fail a safe navigation or screenshot because of those blocked writes.
