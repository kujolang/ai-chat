const { createBrowserContainment } = require("./browser-containment");
...
function createBrowserRuntime(options = {}) {
	...
	const maxSessions = clampInteger(options.maxSessions, 1, 128, 32);
	const maxSessionsPerScope = clampInteger(options.maxSessionsPerScope, 1, 32, 8);
	const maxActionsPerSession = clampInteger(options.maxActionsPerSession, 1, 500, 60);
	const maxActionsPerRequest = clampInteger(options.maxActionsPerRequest, 1, 128, 24);
	...
	const containment = enabled ? createBrowserContainment({ headless, artifactDir }) : null;
	const containmentAvailable = Boolean(containment?.status().available);
	...
	function status() {
		return {
			enabled,
			available: chromiumAvailable && containmentAvailable && !shuttingDown,
			containment: containment?.status() || { available: false, reason: "disabled" },
			backend: chromiumAvailable ? "playwright-chromium" : null,
			headless,
			action_policy: actionPolicy,
			allowlist_enabled: allowedHosts.length > 0,
			unavailable_reason: unavailableReason || (!containmentAvailable ? containment?.status().reason : null) || null
		};
	}
	...
	async function open(args, scopeId, requestState, signal) {
		...
		const reusableSession = reusableScopeSession(scopeId, requestState);
		if (reusableSession) {
			consumeSessionAction(reusableSession);
			await navigate(reusableSession, args.url, signal);
			return boundedResult(await snapshotSession(reusableSession, false, false, signal));
		}
		...
		// HTTP request interception does not own WebSocket traffic. Keep this
		// separate channel closed rather than relying on Chromium's local-network
		// heuristics to enforce the app's HTTP-only policy.
		...
		requestState.defaultSessionId = session.id;
		context.on("page", (candidate) => {
			if (candidate !== page) void candidate.close().catch(() => {});
		});
		await context.route("**/*", async (route) => handleRoute(route, session, session.signal));
	...
	async function browserUse(args, scopeId, requestState, signal) {
		...
		if (args.url && !isHttpUrl(args.url)) {
			return withBrowserUseGuidance("browser_url_blocked", "Only HTTP and HTTPS browser URLs are allowed. Browser tools cannot inspect local files, filesystem paths, file: URLs, about: URLs, or app-internal pages.");
		}
		...
		if ((action === "extract" || action === "snapshot" || action === "screenshot") && !args.session_id && !args.url) {
			const reusableSession = reusableScopeSession(scopeId, requestState);
			if (!reusableSession) {
				return withBrowserUseGuidance("browser_session_required", "No browser session is open. Call browser_open with an absolute public http:// or https:// URL first, or pass a URL to browser_use.");
			}
			if (action === "screenshot") {
				const result = await act({ session_id: reusableSession.id, action: { type: "screenshot" } }, scopeId, requestState, signal);
				return { ...result, deprecated_tool: true };
			}
			const result = await snapshot({ session_id: reusableSession.id }, scopeId, requestState, signal);
			return { ...result, deprecated_tool: true };
		}
		if ((action === "extract" || action === "snapshot" || action === "screenshot") && !args.session_id && args.url) {
	...
	function withBrowserUseGuidance(code, message) {
		return {
			ok: false,
			code,
			message,
			deprecated_tool: true,
			retry_hint: "For web pages, use browser_open first, then browser_snapshot/browser_act/browser_close with the returned session_id. For local repository files such as README.md, use local_workspace_list and local_file_read instead."
		};
	}
	...
	async function closeTool(args, scopeId) {
		...
		if (!session) {
			...
			return { ok: true, session_id: id, closed: true, already_closed: true };
		}
		if (session.scopeId !== scopeId) {
			const error = browserSessionNotFoundError();
			error.retryable = false;
			error.retry_hint = "Use a browser session_id that was opened in this chat scope.";
			throw error;
		}
		await closeSession(session);
	...
	const { safeRequest, validateUrl } = createReadOnlyHttpClient({
		resolveHost, allowPrivateHosts: [...allowPrivateHosts], allowedHosts, navigationTimeoutMs, maxResourceBytes
	});
	...
	function requireSession(rawId, scopeId) {
		...
		if (!session || session.scopeId !== scopeId) {
			throw browserSessionNotFoundError();
		}
		if (nowMs() - session.lastUsedAt > sessionTtlMs) {
			void closeSession(session);
			const error = toolError("browser_session_expired", "The browser session has expired.");
			error.retryable = true;
			error.retry_hint = "Open a new browser session with browser_open and repeat the intended browser action.";
			throw error;
		}
		return session;
	}
	...
	function reusableScopeSession(scopeId, requestState) {
		const sessionId = String(requestState.defaultSessionId || "").trim();
		if (!sessionId) return null;
		const session = sessions.get(sessionId);
		if (!session || session.closed || session.scopeId !== scopeId) return null;
		return session;
	}
	...
	async function getBrowser() {
		...
		if (!browserPromise) {
			browserPromise = playwrightModule.chromium.launch({
				headless,
				...containment.launchOptions(),
				downloadsPath: artifactDir
			}).catch(() => {
				browserPromise = null;
				containment.close();
				throw toolError("browser_containment_unavailable", "Contained Chromium could not be started. Check the installed headless shell and operating-system sandbox.");
			});
		}
	...
	function ensureAvailable() {
		...
		if (!containmentAvailable) throw toolError("browser_containment_unavailable", "An enforced browser network sandbox is unavailable for this platform or mode.");
...
// Shared transport for browser interception and static page extraction. DNS
// results are checked and pinned to each connection; redirects repeat policy.
function createReadOnlyHttpClient(options = {}) {
	const resolveHost = options.resolveHost || defaultResolveHost;
	const allowPrivateHosts = new Set((options.allowPrivateHosts || []).map((host) => String(host).toLowerCase()));
	const allowedHosts = normalizeAllowedHosts(options.allowedHosts || []);
	const navigationTimeoutMs = clampInteger(options.navigationTimeoutMs, 1000, 120000, 15000);
	const maxResourceBytes = clampInteger(options.maxResourceBytes, 1024, 20 * 1024 * 1024, 5 * 1024 * 1024);

	async function safeRequest(rawUrl, requestOptions, redirectCount, signal) {
		throwIfAborted(signal);
		if (!["GET", "HEAD"].includes(requestOptions.method)) throw toolError("browser_navigation_blocked", "Only read-only HTTP methods are allowed.");
		...
		const result = await new Promise((resolve, reject) => {
			const request = transport.request(parsed, {
				...
				response.on("aborted", () => reject(toolError("browser_execution_failed", "The upstream response closed before completion.")));
	...
	return { safeRequest, validateUrl };
}
...
function sanitizeBrowserError(error) {
	...
	if (message.includes("ERR_BLOCKED_BY_CLIENT")) {
		return toolError("browser_url_blocked", "The browser navigation was blocked by the browser safety or network policy.");
	}
	if (message.includes("ERR_ABORTED")) {
		return toolError("browser_navigation_failed", "The browser navigation did not complete.");
	}
	if (/page\.goto:.*timeout/i.test(message)) {
		return toolError("browser_navigation_timeout", "Browser navigation timed out.");
	}
...
function browserSessionNotFoundError() {
	const error = toolError("browser_session_not_found", "The browser session was not found.");
	error.retryable = true;
	error.retry_hint = "Open a new browser session with browser_open and repeat the intended browser action.";
	return error;
}
