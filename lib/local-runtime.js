...
function createLocalRuntime(options = {}) {
	...
	function writeFile(rawArguments = {}) {
		...
		if (isSensitivePath(file) || !readableExtensions.has(path.extname(file).toLowerCase())) {
			throw localError("local_file_write_blocked", "Local writes are limited to non-sensitive text files with known extensions.");
		}
	...
	async function runCommand(rawArguments = {}, context = {}) {
		...
		if (!shellAllowlist.includes(args.command)) {
			throw localError("local_shell_command_blocked", "The requested command is not in AI_CHAT_LOCAL_SHELL_ALLOWLIST.");
		}
...
function resolvePathForRead(root, requestedPath, options = {}) {
	...
	throw localError("local_path_type_mismatch", "The requested path type is not supported for this tool.");
}
