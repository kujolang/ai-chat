	...
	function writeFile(rawArguments = {}, context = {}) {
		...
		try {
			const flags = fs.constants.O_WRONLY | (fs.constants.O_NOFOLLOW || 0)
				| (args.mode === "create" ? fs.constants.O_CREAT | fs.constants.O_EXCL
					: args.mode === "append" ? fs.constants.O_CREAT | fs.constants.O_APPEND : 0);
			descriptor = fs.openSync(file, flags, 0o600);
			if (!fs.fstatSync(descriptor).isFile()) {
				throw localError("local_file_write_blocked", "Local writes require a regular file.");
			}
			if (args.mode === "overwrite") fs.ftruncateSync(descriptor, 0);
...
function resolvePathForWrite(root, requestedPath) {
	...
	// lstat sees dangling links; existsSync follows them and would silently
	// validate only the parent before the write follows a link outside the root.
	const existingAnchor = nearestExistingPath(target);
	let realAnchor = "";
	try {
	...
	const resolvedTarget = path.resolve(realAnchor, path.relative(existingAnchor, target));
	if (isSensitivePath(target) || isSensitivePath(resolvedTarget)) {
		throw localError("local_file_write_blocked", "Sensitive local paths are not writable through chat tools.");
	}
	...
	while (true) {
		try {
			...
			if (error.code !== "ENOENT" && error.code !== "ENOTDIR") {
				throw localError("local_path_blocked", "The target path could not be safely inspected.");
			}
		}
