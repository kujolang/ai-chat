...
const entryPoints = new Set(["system_time", "web_search", "skill_list", "skill_read", "local_workspace_list", "local_file_read", "browser_open", "action_adapter_list"]);
...
function capabilityInstructions(tools) {
	...
	if (names.has("web_search")) lines.push("Use web_search for current facts and source discovery. Search before opening a browser when snippets suffice. If search fails, use browser_open on a known relevant public URL when available; report the failed search without inventing results.");
