...
const { prepareSchemaValue } = require("./tool-input-repair");
...
function createActionRuntime(options = {}) {
	...
	async function call(rawArguments = {}, context = {}) {
		...
		const preparedInput = prepareSchemaValue(`action_adapter_call:${adapter.id}`, args.input, adapter.input_schema, "input");
		if (preparedInput.repairs.length > 0 && typeof context.reportInputRepairs === "function") {
			context.reportInputRepairs(preparedInput.repairs);
		}
		...
		try {
			const response = await fetchFn(adapter.url, {
				...
				body: JSON.stringify({ input: preparedInput.value }),
