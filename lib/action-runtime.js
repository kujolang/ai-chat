...
const { validateThenRepairToolCall } = require("./tool-input-repair");
...
function createActionRuntime(options = {}) {
	...
	async function call(rawArguments = {}, context = {}) {
		...
		const preparedInput = validateThenRepairToolCall({
			name: `action_adapter_call:${adapter.id}`,
			input: args.input,
			schema: adapter.input_schema
		});
		const inputRepairMetadata = preparedInput.repaired ? {
			repaired: true,
			repair_count: preparedInput.notes.length,
			repair_types: preparedInput.repair_types,
			notes: preparedInput.notes
		} : null;
		if (preparedInput.repaired && typeof context.onToolInputRepair === "function") {
			context.onToolInputRepair(inputRepairMetadata);
		}
		...
		try {
			const response = await fetchFn(adapter.url, {
				...
				body: JSON.stringify({ input: preparedInput.input }),
				signal: controller.signal
			});
			if (Number(response.status) >= 300 && Number(response.status) < 400) {
				throw actionError("action_adapter_redirect_blocked", "Action adapter redirects are not allowed.");
			}
			const raw = await readBoundedResponse(response, maxResultBytes, "action_adapter_result_too_large");
			...
			return {
				...
				...(inputRepairMetadata ? { action_input_repair: inputRepairMetadata } : {})
			};
		} catch (error) {
