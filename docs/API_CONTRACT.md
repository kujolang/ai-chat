...
- `credential_managed` is also `true` for Hermes profiles that use the loopback Hermes proxy and its existing Nous Portal login.
...
- Hermes profiles use the exact configured loopback `HERMES_PROXY_URL`; AI Chat never receives the Nous Portal credential held by Hermes.
