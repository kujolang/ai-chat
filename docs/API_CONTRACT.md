...
Built-in arguments use validate-then-repair: schema-valid input is passed through unchanged, while invalid input may receive at most 24 deterministic repairs. Supported repairs are optional-null omission, declared camelCase/legacy aliases, exact scalar string coercion, JSON-string object/array parsing, bare-string-to-array wrapping, empty-object array placeholders, and narrow Markdown auto-link cleanup for path fields. JSON arrays are parsed before bare strings are wrapped. The Action Adapter's nested `input` is validated against its manifest schema by the same layer. Unknown fields, ambiguous coercions, missing required fields, and inputs still invalid after repair return `invalid_tool_arguments` with value-free field/type guidance. Successful tool results may include bounded `input_repairs` entries (`path` and `kind` only); Watchdog and the weekly audit record aggregate repair counts without argument values.
