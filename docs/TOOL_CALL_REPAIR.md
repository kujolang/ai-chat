...
Action Adapter calls apply the same boundary again to their nested `input` using
the selected adapter's manifest schema. Nested repairs are surfaced as
`action_input_repair`; invalid nested input never reaches the loopback service.
