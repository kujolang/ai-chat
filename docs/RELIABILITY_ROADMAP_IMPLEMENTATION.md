...
## September 7 soak B review — incomplete

The original PID `31652` is absent and execution handle `1350` is unavailable. The saved `status.json` still says running, but it is stale: there is no terminal `summary.json`. Last metric evidence is September 6 at 20:28:31.700 UTC, 8,053,326 ms (about 2 hours 14 minutes) after launch. The log ends after attempt 90 without a termination reason. The cause is unknown; this evidence does not establish an application crash or a successful shutdown.

Partial results: 90 attempts, 78 completed responses, 70 correct task checks. GLM eager passed 23/23 and deferred 21/22; Grok eager passed 15/23 and deferred 11/22. Errors include 11 deferred Grok tool-iteration limits and one deferred GLM unavailable-tool result. Metrics contain 1,701 records, including 16 EAGAIN sampling failures; largest successful-sample gap was 15,003 ms. Recorded maximum process-tree RSS was 280,928,256 bytes; warm/final five-minute median RSS was 132,612,096/122,212,352 bytes. No SSE overflow was recorded, and the last pending-byte count was zero. These observations apply only to the partial recorded interval; they do not prove all-day stability.

Item 10 remains incomplete. Preserve `data/reliability-soak-20260906-b/` and `/tmp/ai-chat-soak-b.log`. No replacement live run was started during this review. A later authorized rerun needs a process lifecycle that survives the chat session and host availability for the full interval; the original interruption cause must not be guessed.
